...
function render() {
  ...
  const sceneOneStart = 2.50;
  const sceneTwoStart = 6.35;
  const sceneThreeStart = 10.70;
  const sceneFourStart = 15.10;
  const progress = clamp(scrollUnits / 2.20);
  const signalProgress = clamp((scrollUnits - sceneOneStart) / 3.60);
  const peopleProgress = clamp(((scrollUnits - sceneTwoStart) / 4.32) * .90, 0, .90);
  const peopleEntry = peopleProgress / .90;
  const businessProgress = clamp(((scrollUnits - sceneThreeStart) / 4.00) * .90, 0, .90);
  const datahubProgress = clamp((scrollUnits - sceneFourStart) / 3.60);
